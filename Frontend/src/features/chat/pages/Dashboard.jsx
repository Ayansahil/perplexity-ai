...
// Nav items — clicking Discover → /, History → /history
const NAV_ITEMS = [
...
// Sidebar
const Sidebar = ({ user, chats, currentChatId, onOpenChat, onDeleteChat, onNewChat, onNavigate }) => {
  const navigate = useNavigate()

  return (
    <aside className="h-screen w-64 fixed left-0 top-0 bg-[#131313] flex flex-col py-6 px-4 z-50 border-r border-white/[0.04]">
      {/* Brand */}
      ...
      <div className="mt-auto space-y-0.5">
        ...
        <button
          ...
          className="w-full mt-3 flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 transition-colors group"
        >
...
// TopBar
const TopBar = () => (
  <header className="fixed top-0 right-0 w-[calc(100%-16rem)] h-14 px-8 flex justify-between items-center bg-[#0e0e0e]/80 backdrop-blur-xl z-40 border-b border-white/[0.04]">
...
// Layout — parent route, renders Sidebar + TopBar + <Outlet />
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#0e0e0e] text-white font-['Inter']">
      ...
      <main className="ml-64 flex-1 relative flex flex-col bg-[#0e0e0e] overflow-hidden">
        <TopBar />
        {/* Child view fills the area below the fixed TopBar */}
